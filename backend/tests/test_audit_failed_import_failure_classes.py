...
def test_failure_classes_are_separated_without_authorizing_retry():
    ...
    assert result == {
        ...
        "failed_jobs_zero_activity": 3,
        "zero_activity_known_provenance_failures": 2,
        "zero_activity_unclassified_failures": 1,
        ...
        "manual_reconciliation_required": 5,
...
def test_clean_history_has_no_failed_jobs_and_still_does_not_authorize_writes():
    ...
    assert result["failed_jobs_zero_activity"] == 0
    assert result["schema_cache_retry_candidates"] == 0
    assert result["manual_source_missing_jobs"] == 0
    assert result["manual_reconciliation_required"] == 0
    assert result["safe_automatic_retry_candidates"] == 0
    assert result["automatic_retry_authorized"] is False
    assert result["database_write_authorized"] is False


def test_zero_activity_duplicate_is_not_misclassified_as_safe_retry():
    result = summarize_failed_import_failure_classes(
        [
            {
                "status": "failed",
                "last_error": "manual_source_duplicate:another.pdf",
                "records_imported": 0,
                "records_updated": 0,
                "pages_needing_ocr": [],
            }
    ...
    assert result["failed_jobs_zero_activity"] == 1
    assert result["zero_activity_known_provenance_failures"] == 1
    assert result["manual_reconciliation_required"] == 1
    assert result["safe_automatic_retry_candidates"] == 0
    assert result["automatic_retry_authorized"] is False
