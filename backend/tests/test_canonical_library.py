...
def record(identifier, **changes):
    value = {
        ...
        "name": "Azione",
        "normalized_name": "azione",
        "parent_class": "Guerriero",
        "parent_subclass": "",
        "level": "2",
        "description": "descrizione",
        "full_text": "testo della regola",
        "attributes": {},
        "source_refs": [],
...
def test_grouping_authority_and_character_sheet_exclusion():
    assert canonical_group_key(record("a")).startswith(
        "dnd5e:2014:owner:class_feature:azione"
    )
    ranks = [
        source_authority(
            record("a", source_refs=[{"authority_class": "official_errata"}])
        )[0],
        source_authority(
            record("b", source_refs=[{"authority_class": "official_revision"}])
        )[0],
    ...
    assert is_character_sheet_source(
        record("a", source_key="Scheda_personaggio_template.pdf")
    )
    assert (
        record_ruleset(
            record("a", source_refs=[{"filename": "Player_Handbook_2024.pdf"}])
        )
        == "2024"
    )
...
def test_unregistered_exact_copy_of_registered_manual_is_not_discovered(
    ...
    (tmp_path / "same-manual-with-another-name.pdf").write_bytes(
        b"registered manual bytes"
    )
...
def test_ai_cannot_override_a_higher_authority_source():
    errata = record(
        "errata",
        source_refs=[{"authority_class": "official_errata"}],
        full_text="official",
    )
    ...
    row = asyncio.run(
        canonicalize_group(
            [errata, derived],
            comparator=lambda candidates: {
                "selected_source_record_id": "derived",
                "confidence": 0.99,
                "notes": "wrong priority",
                "conflict_fields": [],
                "status": "verified",
            },
        )
...
def test_invalid_or_low_confidence_ai_never_certifies_and_protects_use():
    ...
    assert not reference_is_trusted(
        {"review_status": "verified", "ai_review_status": "conflict"}
    )
    assert not reference_is_trusted(
        {
...
def test_apply_endpoint_rejects_uncertain_canonical_record():
    ...
    db.private_reference_records.rows = [
        record(
            "uncertain", review_status="verified", ai_review_status="low_confidence"
        ),
    ]
...
def test_in_memory_batch_persists_only_live_canonical_columns_and_resumes():
    ...
    db.private_reference_records.rows = [
        record(
            "old",
            full_text="old",
            source_refs=[{"filename": "Manuale_del_giocatore.pdf", "page": 1}],
        ),
        record(
            "new",
            full_text="new",
            source_key="Errata.pdf",
            source_refs=[
                {
                    "filename": "Errata.pdf",
                    "page": 2,
                    "authority_class": "official_errata",
                }
            ],
        ),
    ]
    ...
    assert next(
        item for item in db.private_reference_records.rows if item["id"] == "new"
    )["ai_review_corrections"]["selected"]
    ...
    db.private_reference_records.rows.append(
        record("next", name="Difesa", normalized_name="difesa")
    )
