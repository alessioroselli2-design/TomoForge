...
def _page_text(title: str, language: str = "en") -> str:
    if language == "en":
        return (
            f"{title}\nThis is original English rules text. "
            + "It remains in English and describes the rule exactly. " * 3
        )
    return (
        f"{title}\nTesto originale abbastanza lungo. "
        + "Descrive la regola senza omissioni. " * 4
    )
...
def test_mixed_pdf_ocr_is_page_scoped_and_failure_does_not_abort(tmp_path):
    ...
    assert all(
        ...
        if record["name"] == "Ocr Rule"
    )
    assert all(
        "ocr_da_verificare" not in record["review_flags"]
        for record in report.records
        if record["name"] == "Native Rule"
    )
...
def test_ocr_prompts_preserve_original_language(monkeypatch):
    ...
    class Response:
        ...
        def json(self):
            return {
                "choices": [{"message": {"content": _page_text("ARCANE RECOVERY")}}]
            }
    ...
    monkeypatch.setattr(
        library.requests,
        "post",
        lambda _url, **kwargs: captured.append(kwargs) or Response(),
    )
        ...
        [
            {
                **common,
                "id": "one",
                "source_key": "book-one.pdf",
                "source_refs": [{"filename": "book-one.pdf", "page": 1}],
            },
            {
                **common,
                "id": "two",
                "source_key": "book-two.pdf",
                "source_refs": [{"filename": "book-two.pdf", "page": 8}],
            },
        ]
    )
...
def test_auto_accept_does_not_verify_ocr_record(monkeypatch, tmp_path):
    ...
    extracted = {
        ...
        "reference_type": "other",
        "name": "OCR Rule",
        "normalized_name": "ocr rule",
        "description": "OCR description",
        "full_text": "OCR complete source text",
        "attributes": {},
        "tags": [],
