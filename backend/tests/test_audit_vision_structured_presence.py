...
def test_presence_counts_do_not_claim_ocr_completion():
    ...
    assert result["registry_structured_presence_complete"] is False
    assert result["parser_ocr_completion_percent"] is None
    assert result["parser_ocr_completion_inferable_from_registry_counts"] is False
    assert result["parser_ocr_readiness_status"] == "NOT_INFERABLE_FROM_REGISTRY_COUNTS"
...
def test_presence_normalizes_registry_metadata_without_upgrading_semantics():
    ...
    assert result["sources_with_structured_records"] == 1
    assert result["structured_presence_percent"] == 100.0
    assert result["registry_structured_presence_complete"] is True
    assert result["parser_ocr_completion_percent"] is None
    assert result["parser_ocr_completion_inferable_from_registry_counts"] is False


def test_empty_scope_is_reported_without_claiming_ocr_readiness():
    result = summarize_vision_structured_presence([])

    assert result["active_catalogued_vision_or_mixed_sources"] == 0
    assert result["structured_presence_percent"] is None
    assert result["registry_structured_presence_complete"] is True
    assert result["parser_ocr_completion_percent"] is None
    assert result["parser_ocr_completion_inferable_from_registry_counts"] is False
