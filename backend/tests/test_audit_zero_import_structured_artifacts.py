...
def test_structured_artifact_audit_is_conservative():
    ...
    assert result["source_filename_identity_is_not_unique_provenance"] is True
...
def test_duplicate_live_source_filename_never_becomes_strong_review_evidence():
    sources = [
        _source("dup-a", "Same-Book.pdf", 75),
        _source("dup-b", "Same-Book.pdf", 75),
    ]
    analyses = [
        {"filename": "Same-Book.pdf", "pages": 75, "total_characters": 9000},
    ]

    result = summarize_zero_import_structured_artifacts(sources, analyses)

    assert result["zero_import_vision_sources_total"] == 2
    assert result["zero_import_sources_with_ambiguous_source_identity"] == 2
    assert result["source_ids_with_ambiguous_source_identity"] == ["dup-a", "dup-b"]
    assert result["zero_import_sources_with_exact_historical_text_review_evidence"] == 0
    assert result["source_ids_with_exact_historical_text_review_evidence"] == []
    assert result["historical_evidence_triage_partition_complete"] is True
