...
def run(record_value, answer):
    return asyncio.run(
        verify_translation(
            record_value, comparator=lambda _record: answer, model="test-model"
        )
    )
...
def test_incomplete_source_snapshot_is_deterministic_conflict():
    result = asyncio.run(
        verify_translation(record(source_full_text=""), comparator=lambda _r: {})
    )
    assert result["status"] == TRANSLATION_CONFLICT
...
def test_italian_source_does_not_require_translation_verification():
    result = asyncio.run(
        verify_translation(
            record(source_language="it", translation_status="not_required")
        )
    )
...
def test_provider_or_invalid_verdict_failure_is_not_certified():
    ...
    result = asyncio.run(
        verify_translation(record(), comparator=broken, model="test-model")
    )
...
def test_fingerprint_changes_when_source_or_translation_changes():
    ...
    assert first != translation_verification_fingerprint(
        record(source_full_text=original["source_full_text"] + " Extra.")
    )
    assert first != translation_verification_fingerprint(
        record(full_text=original["full_text"] + " Extra.")
    )
...
def test_prompt_is_strictly_translation_fidelity_only():
    prompt = build_translation_verification_prompt(
        record(review_flags=["ocr_da_verificare"])
    )
...
def test_mechanical_tokens_keep_dice_and_numbers_stable():
    assert mechanical_tokens(
        "CD 15, 1d8 + 3 danni, 30 piedi e 50%"
    ) == mechanical_tokens("DC 15, 1d8 + 3 damage, 30 feet and 50%")
