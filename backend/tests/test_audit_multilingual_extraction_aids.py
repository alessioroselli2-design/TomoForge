...
def test_cross_language_extraction_aid_is_reported_without_authorizing_translation_or_import():
    ...
    assert (
        result["zero_import_difficult_sources_without_multilingual_extraction_aid"]
        == []
    )
    assert (
        result["text_only_extraction_aid_requires_structured_parse_before_record_reuse"]
        is True
    )
    ...
    assert (
        result["cross_language_peer_does_not_replace_authoritative_language_source"]
        is True
    )
...
def test_language_suffixed_family_rejects_language_metadata_mismatch():
    ...
    assert result[
        "zero_import_difficult_sources_without_multilingual_extraction_aid"
    ] == ["book-it"]
...
def test_same_language_copy_is_not_classified_as_multilingual_extraction_aid():
    ...
    assert result["multilingual_extraction_aid_peer_ids_by_source"] == {}
    assert (
        result["zero_import_difficult_sources_with_multilingual_extraction_aid"] == []
    )
    assert result[
        "zero_import_difficult_sources_without_multilingual_extraction_aid"
    ] == ["book-a"]
...
def test_only_catalogued_zero_import_difficult_sources_are_triaged():
    ...
    result = summarize_multilingual_extraction_aids(
        [already_imported, text_source, excluded]
    )
