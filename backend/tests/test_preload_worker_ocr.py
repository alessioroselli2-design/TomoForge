...
def _job(page=1, lease="lease-1", unresolved=None):
    return {
        ...
        "attempt_count": 0,
        "last_error": "",
        "pages_needing_ocr": list(unresolved or []),
        "records_imported": 0,
        "records_updated": 0,
        "records_flagged": 0,
        "records_skipped": 0,
...
def _setup(monkeypatch, tmp_path, job, page_count, batch_size, importer):
    ...
    monkeypatch.setattr(
        library, "available_reference_manuals", lambda: {FILENAME: source}
    )
...
def _report(imported=0, unresolved=None):
    return server.ReferenceImportResult(
        imported=imported,
        updated=0,
        flagged_for_review=imported,
        skipped=0,
        sources=[{"filename": FILENAME, "pages_needing_ocr": list(unresolved or [])}],
    )


def test_mixed_native_and_successful_ocr_pages_complete_worker_chunk(
...
def test_failed_or_blank_ocr_page_is_preserved_while_worker_advances(
    monkeypatch, tmp_path
):
    async def importer(*_args, **_kwargs):
...
def test_unresolved_page_in_first_chunk_does_not_block_later_chunks(
    monkeypatch, tmp_path
):
    calls = []
    ...
    asyncio.run(
        preload.process_manual_preload_job(USER_ID, copy.deepcopy(jobs.rows[0]), db=db)
    )
...
def test_real_import_error_keeps_existing_retry_then_failure_semantics(
    monkeypatch, tmp_path
):
    async def importer(*_args, **_kwargs):
    ...
    assert (
        jobs.rows[0]["status"],
        jobs.rows[0]["current_page"],
        jobs.rows[0]["attempt_count"],
    ) == ("queued", 1, 1)
    jobs.rows[0].update(status="processing", lease_id="lease-2")
    asyncio.run(
        preload.process_manual_preload_job(USER_ID, copy.deepcopy(jobs.rows[0]), db=db)
