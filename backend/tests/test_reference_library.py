...
Has entrenado tu cuerpo para llevar armaduras ligeras sin perder movilidad.""",
    ...
    leader = next(
        record for record in report.records if record["name"] == "Líder Inspirador"
    )
...
def test_reference_extractor_uses_ocr_callback_for_unreadable_page(tmp_path):
    ...
    report = extract_reference_records(
        pdf_path,
        ocr_page=lambda page, page_number: (
            """TALENTO DELLA GUERRA
...
def test_gemini_ocr_returns_empty_for_http_transport_and_malformed_responses(
    monkeypatch,
):
...
def test_equipment_card_payloads_map_to_their_dedicated_card_types():
    weapon = reference_to_card_payload(
        make_reference(
            "Spada lunga",
    ...
    armor = reference_to_card_payload(
        make_reference(
            "Armatura di cuoio",
    ...
    item = reference_to_card_payload(
        make_reference(
            "Zaino",
...
def test_subclass_and_class_feature_keep_dedicated_printable_card_types():
    subclass = reference_to_card_payload(
        make_reference("Maestro della battaglia", reference_type="subclass")
    )
    feature = reference_to_card_payload(
        make_reference(
...
"""
    ...
    feature = next(
        record for record in records if record["name"] == "Superiorità In Combattimento"
    )
...
"""
    ...
    assert (
        by_name["Mantello Di Protezione"]["attributes"]["sintonia"]
        == "Richiede sintonia"
    )
...
def test_spell_payload_derives_damage_and_card_action_from_imported_text():
    payload = reference_to_card_payload(
        make_reference(
            "Lama infuocata",
            reference_type="spell",
            attributes={
...
def test_current_ai_translation_verdict_unlocks_only_translation_uncertainty():
    ...
    translated["translation_review_fingerprint"] = translation_verification_fingerprint(
        translated
    )
...
def test_apply_reference_endpoint_accepts_verified_record_with_failed_translation(
    monkeypatch,
):
    ...
    result = asyncio.run(
        server.apply_private_reference(record["id"], user, db=_test_db)
    )
...
def test_apply_reference_endpoint_rejects_unverified_record_with_failed_translation(
    monkeypatch,
):
...
class MemoryReferences:
    ...
    def find(self, query):
        ...
        return SimpleNamespace(
            to_list=lambda limit: asyncio.sleep(0, result=rows[:limit])
        )
...
def test_character_references_derive_source_refs_and_create_only_selected_rule_cards(
    monkeypatch,
):
    ...
    linked = asyncio.run(
        server.create_linked_cards(
            character.id,
            server.LinkedCardInput(
                reference_ids=[record["id"]], version=character.version
            ),
            user,
            db=_test_db,
        )
    ...
    try:
        asyncio.run(
            server.create_linked_cards(
                character.id,
                server.LinkedCardInput(
                    reference_ids=["ref-non-collegato"], version=updated.version
                ),
                user,
                db=_test_db,
            )
...
def test_reference_snapshots_detect_a_corrected_source_and_preserve_manual_character_values(
    monkeypatch,
):
    ...
    assert (
        reference_snapshot(corrected)["content_revision"]
        != saved_snapshot["content_revision"]
    )
    ...
    refreshed = asyncio.run(
        server.refresh_card_reference_updates(
            character.id,
            server.ReferenceUpdateInput(
                reference_ids=[original["id"]], version=character.version
            ),
            user,
            db=_test_db,
        )
    ...
    assert (
        refreshed["card"].reference_snapshots[0]["source_text_checksum"]
        == "versione-corretta"
    )
    assert (
        asyncio.run(server.card_reference_updates(character.id, user, db=_test_db))[
            "updated_count"
        ]
        == 0
    )
...
def test_card_history_keeps_manual_and_user_changes_separate_and_account_scoped(
    monkeypatch,
):
    cards = server.MemoryCollection()
    ...
    manual_saved = asyncio.run(
        server.complete_card_from_manuals(
            character.id,
    ...
    undone = asyncio.run(
        server.undo_card_change(
            character.id,
            server.CardVersionInput(version=manual_saved.version),
            owner,
            db=_test_db,
        )
    )
    ...
    redone = asyncio.run(
        server.redo_card_change(
            character.id,
            server.CardVersionInput(version=undone["card"].version),
            owner,
            db=_test_db,
        )
    )
...
def test_card_history_redo_follows_undo_order_and_drops_stale_branches(monkeypatch):
    ...
    _test_db = SimpleNamespace(
        cards=cards, private_reference_records=server.MemoryCollection()
    )
    ...
    first = asyncio.run(
        server.update_card(
            character.id,
            server.CardUpdate(
                attributes={"pf_attuali": "14"}, version=character.version
            ),
            user,
            db=_test_db,
        )
    ...
    second = asyncio.run(
        server.update_card(
            character.id,
            server.CardUpdate(attributes={"pf_attuali": "9"}, version=first.version),
            user,
            db=_test_db,
        )
    )
    ...
    undo_second = asyncio.run(
        server.undo_card_change(
            character.id,
            server.CardVersionInput(version=second.version),
            user,
            db=_test_db,
        )
    )
    assert undo_second["card"].attributes["pf_attuali"] == "14"
    undo_first = asyncio.run(
        server.undo_card_change(
            character.id,
            server.CardVersionInput(version=undo_second["card"].version),
            user,
            db=_test_db,
        )
    )
    assert undo_first["card"].attributes["pf_attuali"] == "18"
    redo_first = asyncio.run(
        server.redo_card_change(
            character.id,
            server.CardVersionInput(version=undo_first["card"].version),
            user,
            db=_test_db,
        )
    )
    assert redo_first["card"].attributes["pf_attuali"] == "14"
    redo_second = asyncio.run(
        server.redo_card_change(
            character.id,
            server.CardVersionInput(version=redo_first["card"].version),
            user,
            db=_test_db,
        )
    )
    ...
    undo_for_branch = asyncio.run(
        server.undo_card_change(
            character.id,
            server.CardVersionInput(version=redo_second["card"].version),
            user,
            db=_test_db,
        )
    )
    asyncio.run(
        server.update_card(
            character.id,
            server.CardUpdate(
                attributes={"pf_attuali": "7"}, version=undo_for_branch["card"].version
            ),
            user,
            db=_test_db,
        )
    ...
    try:
        asyncio.run(
            server.redo_card_change(
                character.id,
                server.CardVersionInput(version=undo_for_branch["card"].version + 1),
                user,
                db=_test_db,
            )
        )
...
def test_card_update_rejects_a_stale_editor_version_without_changing_history(
    monkeypatch,
):
    cards = server.MemoryCollection()
    _test_db = SimpleNamespace(
    ...
    character = asyncio.run(
        server.create_card(
            server.CardCreate(
                type="character",
                name="Neris",
                attributes={"pf_attuali": "18", "tiri_salvezza": "Forza"},
            ),
            user,
            db=_test_db,
        )
    ...
    saved = asyncio.run(
        server.update_card(
            character.id,
            server.CardUpdate(
                attributes={"pf_attuali": "14", "tiri_salvezza": "Forza"},
    ...
    try:
        asyncio.run(
            server.update_card(
                character.id,
                server.CardUpdate(
                    attributes={"pf_attuali": "18", "tiri_salvezza": "Destrezza"},
                    version=stale_version,
                ),
                user,
                db=_test_db,
...
def test_versioned_card_mutations_reject_stale_concurrent_actions(monkeypatch):
    ...
    manual_card = asyncio.run(
        server.create_card(
            server.CardCreate(type="character", name="Manuale"), user, db=_test_db
        )
    )
    ...
    assert (
        asyncio.run(server.get_card(manual_card.id, user, db=_test_db)).version
        == completed.version
    )

    reference_card = asyncio.run(
        server.create_card(
            server.CardCreate(
                type="character",
                name="Riferimenti",
    ...
    refreshed = asyncio.run(
        server.refresh_card_reference_updates(
            reference_card.id,
            server.ReferenceUpdateInput(
                reference_ids=[reference["id"]], version=reference_card.version
            ),
            user,
            db=_test_db,
        )
    ...
    assert (
        asyncio.run(server.get_card(reference_card.id, user, db=_test_db)).version
        == refreshed["card"].version
    )

    undo_card = asyncio.run(
        server.create_card(
            server.CardCreate(type="character", name="Cronologia"), user, db=_test_db
        )
    )
    changed = asyncio.run(
        server.update_card(
            undo_card.id,
            server.CardUpdate(
                attributes={"pf_attuali": "12"}, version=undo_card.version
            ),
            user,
            db=_test_db,
        )
    ...
    assert (
        asyncio.run(server.get_card(undo_card.id, user, db=_test_db)).version
        == changed_again.version
    )

    redo_card = asyncio.run(
        server.create_card(
            server.CardCreate(type="character", name="Ripristino"), user, db=_test_db
        )
    )
    changed_for_redo = asyncio.run(
        server.update_card(
            redo_card.id,
            server.CardUpdate(
                attributes={"pf_attuali": "12"}, version=redo_card.version
            ),
            user,
            db=_test_db,
        )
    )
    undone_for_redo = asyncio.run(
        server.undo_card_change(
            redo_card.id,
    ...
    refreshed_for_redo = asyncio.run(
        server.refresh_card_reference_updates(
            redo_card.id,
    ...
    assert (
        asyncio.run(server.get_card(redo_card.id, user, db=_test_db)).version
        == refreshed_for_redo["card"].version
    )

    linked_character = asyncio.run(
        server.create_card(
            server.CardCreate(
                type="character",
                name="Carte collegate",
                reference_ids=[reference["id"]],
            ),
            user,
            db=_test_db,
    ...
    linked = asyncio.run(
        server.create_linked_cards(
            linked_character.id,
            server.LinkedCardInput(
                reference_ids=[reference["id"]], version=linked_character.version
            ),
            user,
            db=_test_db,
        )
    ...
    delete_card = asyncio.run(
        server.create_card(
            server.CardCreate(type="character", name="Da eliminare"), user, db=_test_db
        )
    )
    updated_for_delete = asyncio.run(
        server.update_card(
            delete_card.id,
    ...
    assert (
        asyncio.run(server.get_card(delete_card.id, user, db=_test_db)).name
        == "Ancora qui"
    )
    assert asyncio.run(
        server.delete_card(
...
def test_untracked_reference_can_be_baselined_without_changing_card_data(monkeypatch):
    record = make_reference(
        "Passo silenzioso", reference_type="feat", source_text_checksum="prima-versione"
    )
    cards = server.MemoryCollection()
    cards.rows.append(
        server.Card(
    ...
    report = asyncio.run(
        server.card_reference_updates("legacy-character", user, db=_test_db)
    )
...
def test_attribute_only_source_correction_is_detected_and_keeps_edited_linked_entry(
    monkeypatch,
):
    original = make_reference(
        "Guardia vigile",
    ...
    assert (
        refreshed["card"].attributes["privilegi"][0]["descrizione"]
        == "Nota personale: la mia versione al tavolo."
    )
...
def test_apply_reference_endpoint_cannot_read_another_users_record(monkeypatch):
    ...
    try:
        asyncio.run(
            server.apply_private_reference(record["id"], other_user, db=_test_db)
        )
...
def test_unverified_references_cannot_be_attached_or_materialized_as_linked_cards(
    monkeypatch,
):
    record = make_reference(
        "Privilegio OCR",
...
def test_generate_content_labels_manual_content_without_a_trusted_source(monkeypatch):
    ...
    user = server.User(
        user_id="owner-1", email="mago@example.com", name="Mago", premium_manual=True
    )
...
def test_library_search_returns_sourced_or_explicitly_unavailable(monkeypatch):
    ...
    _test_db = SimpleNamespace(
        private_reference_records=MemoryReferences([trusted, unverified])
    )

    user = server.User(user_id="owner-1", email="mago@example.com", name="Mago")

    sourced = asyncio.run(
        server.search_private_library(
            q="Palla di fuoco", types="spell", user=user, db=_test_db
        )
    )
    unavailable = asyncio.run(
        server.search_private_library(
            q="Dardo incantato", types="spell", user=user, db=_test_db
        )
    )
...
def test_library_review_search_scopes_manuals_and_filters_before_result_limit(
    ...
    _test_db = SimpleNamespace(
        private_reference_records=MemoryReferences(
            [
                *verified_records,
                review_in_a,
                review_in_b,
...
def test_manual_coverage_counts_valid_missing_and_records_to_review(
    monkeypatch, tmp_path
):
...
def test_coverage_endpoint_includes_translation_pending_in_totals(
    monkeypatch, tmp_path
):
    ...
    monkeypatch.setattr(
        lib_mod, "MANUAL_COVERAGE_CATEGORIES", {"Manuale.pdf": ("class",)}
    )
...
def test_translation_review_shows_private_comparison_and_unlocks_only_after_confirmation(
    monkeypatch,
):
    record = make_reference(
        "Barbaro",
        ...
        source_refs=[
            {"filename": "Manual del Jugador.pdf", "page": 46, "language": "es"}
        ],
        translation_status="translated",
        review_status="pending",
    )
    ...
    details = asyncio.run(
        server.get_private_reference_review(record["id"], owner, db=_test_db)
    )
    assert details["original"]["name"] == "Bárbaro"
    assert (
        details["original"]["full_text"] == "Un guerrero feroz que combate con furia."
    )
    assert details["translation"]["name"] == "Barbaro"
    assert (
        details["translation"]["full_text"]
        == "Un guerriero feroce che combatte con furia."
    )
    assert details["manual"] == [
        {"filename": "Manual del Jugador.pdf", "page": 46, "language": "es"}
    ]

    with pytest.raises(server.HTTPException) as other_owner:
        asyncio.run(
            server.get_private_reference_review(record["id"], other_user, db=_test_db)
        )
    assert other_owner.value.status_code == 404

    rejected = asyncio.run(
        server.review_private_reference(
            record["id"],
    ...
    assert (
        rejected["translation"]["full_text"]
        == "Un guerriero feroce corretto che combatte con furia."
    )
    ...
    assert (
        rejected["review_history"][0]["review_notes"]
        == "Applicata la correzione; richiede ancora conferma."
    )
    ...
    applied = asyncio.run(
        server.apply_private_reference(record["id"], owner, db=_test_db)
    )
...
def test_concurrent_translation_reviews_append_every_decision(monkeypatch):
    ...
    assert {entry["review_status"] for entry in details["review_history"]} == {
        "needs_review",
        "verified",
    }
...
def test_needs_review_status_does_not_clear_translation_error(monkeypatch):
    ...
    asyncio.run(
        server.review_private_reference(
            record["id"],
            server.ReferenceReviewInput(
                review_status="needs_review", review_notes="Da ricontrollare."
            ),
            owner,
            db=_test_db,
        )
...
def test_same_source_import_uses_distinct_ids_for_distinct_owners(
    monkeypatch, tmp_path
):
    source = tmp_path / "Manuale.pdf"
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: SimpleNamespace(
            records=[record], pages_read=1, pages_needing_ocr=[]
        ),
    )
...
def test_ocr_import_requires_server_side_confirmation_and_one_manual(
    monkeypatch, tmp_path
):
    ...
    monkeypatch.setattr(
        lib_mod,
        "available_reference_manuals",
        lambda: {"Primo.pdf": first, "Secondo.pdf": second},
    )

    try:
        asyncio.run(
            server.import_private_reference_manuals(
                "owner-1",
...
def test_ocr_import_sends_pages_to_openai_not_gemini(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        lib_mod, "available_reference_manuals", lambda: {pdf_path.name: pdf_path}
    )
...
def test_manual_source_duplicate_detection_compares_required_distinct_assets(
    monkeypatch, tmp_path
):
...
def test_spanish_translation_uses_only_structured_fields_and_requires_complete_json(
    monkeypatch,
):
    captured = {}
...
"""
    ...
    assert (
        translated["ref-es"]["full_text"]
        == "Un guerriero feroce con tutto il testo tradotto."
    )
...
def test_automatic_preload_keeps_successful_translations_unverified(
    monkeypatch, tmp_path
):
    ...
    monkeypatch.setattr(
        lib_mod, "available_reference_manuals", lambda: {filename: source}
    )
    monkeypatch.setattr(
        lib_mod,
        "extract_reference_records",
...
def test_spanish_import_reuses_translation_and_keeps_source_text(monkeypatch, tmp_path):
    ...
    first = asyncio.run(
        server.import_private_reference_manuals("owner-1", body, db=_test_db)
    )
    second = asyncio.run(
        server.import_private_reference_manuals("owner-1", body, db=_test_db)
    )
...
def test_spanish_translation_failure_is_saved_for_review(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        "extract_reference_records",
        lambda *args: SimpleNamespace(
            records=[make_reference("Bárbaro", reference_type="class")],
            pages_read=1,
            pages_needing_ocr=[],
        ),
    )
    monkeypatch.setattr(
        lib_mod,
        "translate_spanish_reference_batch",
        lambda batch: ({}, "provider_translation_failed"),
    )
...
def test_translate_spanish_batch_returns_rate_limited_when_both_providers_return_429(
    monkeypatch,
):
...
def test_translate_spanish_batch_rate_limited_when_gemini_429_and_openai_fails(
    monkeypatch,
):
...
def test_translate_spanish_batch_rate_limited_when_openai_429_after_gemini_fails(
    monkeypatch,
):
...
def test_spanish_translation_rate_limit_saves_record_without_review_flag(
    monkeypatch, tmp_path
):
...
def test_rate_limited_batch_retries_each_record_individually_and_saves_successes(
    monkeypatch, tmp_path
):
    ...
    monkeypatch.setattr(
        ...
        "extract_reference_records",
        lambda *args: SimpleNamespace(
            records=[barbaro, ladron], pages_read=1, pages_needing_ocr=[]
        ),
    )
    monkeypatch.setattr(
        lib_mod, "translate_spanish_reference_batch", lambda batch: next(responses)
    )
...
def test_rate_limited_singleton_batch_is_not_retried_individually(
    monkeypatch, tmp_path
):
    ...
    monkeypatch.setattr(
        ...
        "extract_reference_records",
        lambda *args: SimpleNamespace(
            records=[barbaro], pages_read=1, pages_needing_ocr=[]
        ),
    )
...
def test_rate_limited_batch_individual_retry_content_error_marks_record_for_review(
    monkeypatch, tmp_path
):
...
def test_rate_limited_batch_short_circuits_remaining_records_after_first_individual_429(
    monkeypatch, tmp_path
):
    ...
    monkeypatch.setattr(
        ...
        "extract_reference_records",
        lambda *args: SimpleNamespace(
            records=[barbaro, ladron, guerrero], pages_read=1, pages_needing_ocr=[]
        ),
    )
    monkeypatch.setattr(lib_mod, "translate_spanish_reference_batch", mock_translate)
    ...
    for name in ("Bárbaro", "Ladrón", "Guerrero"):
        ...
        assert row["translation_error"] == "provider_rate_limited", (
            f"{name} should be provider_rate_limited"
        )
...
def test_subsequent_batches_skipped_when_provider_still_limited_after_individual_retry(
    monkeypatch, tmp_path
):
...
def test_subsequent_batches_skipped_when_singleton_batch_is_rate_limited(
    monkeypatch, tmp_path
):
    ...
    def mock_translate(batch):
        ...
        raise AssertionError(
            f"Unexpected translate call #{call_count[0]} — second batch must be skipped"
        )
...
def test_rate_limit_retry_uses_backoff_delays_and_escalates_to_review_when_exhausted(
    monkeypatch,
):
...
def test_preload_worker_sets_rate_limit_status_and_triggers_retry(
    monkeypatch, tmp_path
):
    ...
    async def fake_retry_rate_limited(
        user_id, filename, collection, delays=None, job_updater=None, *, db=None
    ):
    ...
    monkeypatch.setattr(
        lib_mod, "available_reference_manuals", lambda: {source.name: source}
    )
    ...
    monkeypatch.setattr(
        preload_mod, "_retry_rate_limited_translations", fake_retry_rate_limited
    )

    asyncio.run(
        server.ensure_manual_preload_jobs(
            "owner-1", server.ManualPreloadInput(), db=_test_db
        )
    )
...
def test_rate_limit_retry_processes_all_records_beyond_page_boundary(monkeypatch):
    ...
    assert still_parked == [], (
        f"{len(still_parked)} records remain silently rate-limited"
    )
...
def test_repeated_429_during_backoff_never_adds_review_flag_before_exhaustion(
    monkeypatch,
):
...
def test_retry_rate_limited_does_not_overwrite_user_verified_translation(monkeypatch):
    ...
    assert translate_calls == [], (
        "translate_spanish_reference_batch was called for a verified record"
    )
    ...
    assert stored["description"] == verified_description, (
        "Verified description was overwritten"
    )
...
def test_retry_translation_finalization_skips_record_verified_during_provider_call(
    monkeypatch,
):
    ...
    monkeypatch.setattr(
        lib_mod, "translate_spanish_reference_batch", translate_and_verify
    )
    _test_db = SimpleNamespace(private_reference_records=records)

    asyncio.run(
        server.retry_private_reference_translation(
            "owner-1", "ref-owned-barbaro", db=_test_db
        )
    )
...
def test_concurrent_translation_retries_share_one_provider_call_and_return_completed_record(
    monkeypatch,
):
    source = {
        **make_reference(
    ...
    def translate(batch):
        ...
        assert allow_provider_result.wait(timeout=2), (
            "The test did not release the provider"
        )
        return {
            source["id"]: {
                "name": "Barbaro",
...
def test_retry_translated_record_does_not_call_provider_or_modify_record(monkeypatch):
    ...
    monkeypatch.setattr(
        lib_mod, "translate_spanish_reference_batch", lambda batch: calls.append(batch)
    )
...
def test_retry_translation_endpoint_rejects_non_premium_user_before_provider_call(
    monkeypatch,
):
    calls = []
...
def test_spanish_manual_rejects_ocr_even_when_the_request_is_confirmed(
    monkeypatch, tmp_path
):
    source = tmp_path / "Manual-del-Jugador.pdf"
    source.write_bytes(b"native-text")
...
def test_manual_import_progress_reports_pages_translation_and_review_states():
    records = [
        make_reference(
            ...
            source_language="es",
            source_refs=[
                {"filename": "Manual del Jugador.pdf", "page": 5, "language": "es"}
            ],
            translation_status="translated",
        ),
        make_reference(
            ...
            source_language="es",
            source_refs=[
                {"filename": "Manual del Jugador.pdf", "page": 6, "language": "es"}
            ],
            translation_status="failed",
            review_flags=["traduzione_da_verificare"],
            review_status="needs_review",
...
def test_spanish_translation_requires_consent_before_provider_call(
    monkeypatch, tmp_path
):
    source = tmp_path / "Manual-del-Jugador.pdf"
    source.write_bytes(b"native-text")
...
def test_automatic_preload_queues_translation_and_ocr_without_user_consent(
    monkeypatch, tmp_path
):
...
def test_automatic_preload_marks_a_duplicate_manual_source_failed(
    monkeypatch, tmp_path
):
    monster = tmp_path / "Monster.pdf"
    player = tmp_path / "Player.pdf"
    ...
    assert (
        asyncio.run(
            lib_mod.find_private_reference(
                "owner-1",
                "Mostro errato",
                db=_test_db,
            )
    ...
    monster_coverage = next(
        report for report in coverage if report["filename"] == monster.name
    )
    assert all(
        category["records_total"] == 0 for category in monster_coverage["categories"]
    )


def test_automatic_preload_processes_all_chunks_without_manual_ranges(
    monkeypatch, tmp_path
):
...
def test_automatic_preload_starts_ocr_without_user_consent(monkeypatch, tmp_path):
    ...
    async def fake_import(_owner_id, body, *, db=None):
        calls.append(
            (body.use_ai_ocr, body.external_processing_confirmed, body.auto_accept)
        )
        return server.ReferenceImportResult(
            imported=1,
            updated=0,
...
def test_spanish_preload_skips_unreadable_cover_pages_and_continues(
    monkeypatch, tmp_path
):
    filename = "731764731-D-D-Manual-Del-Jugador-5e_1787286581630.pdf"
...
def test_startup_reclaims_only_expired_preload_leases(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda user_id, *, db=None: started.append(user_id),
    )
...
def test_startup_requeues_legacy_translation_consent_job_for_processing(
    monkeypatch, tmp_path
):
    source = tmp_path / "Manuale-nativo.pdf"
    source.write_bytes(b"manual")
    ...
    monkeypatch.setattr(lib_mod, "manual_page_count", lambda _path: 1)
    monkeypatch.setattr(
        preload_mod, "start_manual_preload_worker", lambda user_id, *, db=None: None
    )
...
def test_startup_requeues_completed_manual_when_its_parser_revision_changes(
    monkeypatch, tmp_path
):
    filename = "731764731-D-D-Manual-Del-Jugador-5e_1787286581630.pdf"
    source = tmp_path / filename
    ...
    assert jobs.rows[0]["status"] == "queued"
    assert jobs.rows[0]["current_page"] == 1
    assert jobs.rows[0]["source_fingerprint"] == server.manual_source_fingerprint(
        source
    )
    assert started == ["owner-1"]
...
"""
    ...
    assert (
        reference_to_card_payload({**records[0], "translation_status": "translated"})[
            "card_type"
        ]
        == "spell"
    )
...
"""
    ...
    assert (
        by_name["Acelerar"]["attributes"]["durata"] == "Concentración, hasta 1 minuto"
    )
...
def test_preload_worker_writes_retry_state_to_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(lib_mod, "import_private_reference_manuals", fake_import)
    monkeypatch.setattr(
        preload_mod, "_retry_rate_limited_translations", capturing_retry
    )
...
def test_gemini_ocr_returns_transcription_for_valid_response(monkeypatch, tmp_path):
    ...
    valid_payload = {"candidates": [{"content": {"parts": [{"text": ocr_text}]}}]}

    monkeypatch.setattr(
        server.requests, "post", lambda *a, **k: _OcrResponse(payload=valid_payload)
    )
    monkeypatch.setattr(lib_mod, "GEMINI_API_KEY", "test-key")
...
def test_extract_reference_records_handles_ocr_only_manual_with_mixed_pages(tmp_path):
    ...
    assert any("Trappola" in n for n in names), (
        "OCR-sourced record from page 2 must be extracted"
    )

    ocr_records = [
        r for r in report.records if "ocr_da_verificare" in r["review_flags"]
    ]
    assert len(ocr_records) >= 1, (
        "Records extracted via OCR must carry ocr_da_verificare"
    )

    native_records = [
        r for r in report.records if "ocr_da_verificare" not in r["review_flags"]
    ]
    assert len(native_records) >= 1, (
        "Records from the native text layer must NOT carry ocr_da_verificare"
    )


def test_preload_worker_completes_with_warning_after_unresolved_ocr(
    monkeypatch, tmp_path
):
    ...
    asyncio.run(
        server.process_manual_preload_job("owner-1", dict(jobs.rows[0]), db=_test_db)
    )
...
def test_preload_worker_accumulates_pages_needing_ocr_across_chunks(
    monkeypatch, tmp_path
):
...
def test_ocr_import_blocks_missing_consent_before_any_page_reaches_gemini(
    ...
    try:
        ...
        assert error.status_code == 400
        assert (
            "Gemini" in error.detail
            or "esplicitamente" in error.detail
            or "pagine selezionate" in error.detail
        )
...
def test_valid_ocr_import_sends_each_selected_page_once_and_persists_review_flags(
    ...
    assert len(collection.rows) == 2
    assert {normalize_reference_name(record["name"]) for record in collection.rows} == {
    ...
    assert all(
        "ocr_da_verificare" in record["review_flags"] for record in collection.rows
    )
    assert not [
        record
        for record in collection.rows
...
def test_changed_source_clears_old_corrections_and_stays_in_review_on_repeat_import(
    monkeypatch, tmp_path
):
    source = tmp_path / "Manuale.pdf"
    source.write_bytes(b"manual")
