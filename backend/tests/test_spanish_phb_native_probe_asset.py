...
def test_spanish_phb_asset_matches_registry_and_has_native_text() -> None:
    ...
    result = bounded_native_text_parser_probe(
        SPANISH_PHB, start_page=1, end_page=12, source_language="es"
    )
...
def test_spanish_phb_spread_probe_reports_structured_output_per_window() -> None:
    ...
    assert (
        ...
        + result["unnamed_records_detected_total"]
        == result["records_detected_total"]
    )
    assert result["productive_windows"] == result["named_signal_windows"] > 0, result
    assert (
        result["productive_windows"] + result["empty_windows"] == result["window_count"]
    )
    assert result["record_types_total"], result
    assert [
        (window["start_page"], window["end_page"]) for window in result["windows"]
    ] == [(1, 3), (255, 257), (509, 511), (763, 765)]
    ...
    for window in result["windows"]:
        assert (
            window["records_detected"]
            == window["named_records_detected"] + window["unnamed_records_detected"]
        )
        assert window["records_detected"] == sum(window["record_types"].values())
        aggregate_types.update(window["record_types"])

    assert result["records_detected_total"] == sum(
        window["records_detected"] for window in result["windows"]
    )
    assert result["named_records_detected_total"] == sum(
        window["named_records_detected"] for window in result["windows"]
    )
    assert result["unnamed_records_detected_total"] == sum(
        window["unnamed_records_detected"] for window in result["windows"]
    )
    ...
    assert ranking == sorted(
        ranking,
        key=lambda window: (
            -window["named_records_detected"],
            window["unnamed_records_detected"],
            window["window_index"],
        ),
    )
