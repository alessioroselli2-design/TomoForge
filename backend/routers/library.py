...
async def private_library_manuals(
    user: User = Depends(require_premium), db: SupabaseDatabase = Depends(get_db)
):
        ...
        manuals.append(
            {
                ...
                "job": manual_preload_summary(
                    jobs_by_filename.get(filename), page_count
                ),
                **progress,
            }
        )
...
async def start_private_library_preload(
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail="Impossibile avviare l'indicizzazione automatica"
        ) from exc


@router.get("/library/coverage")
async def private_library_coverage(
    user: User = Depends(require_premium), db: SupabaseDatabase = Depends(get_db)
):
    ...
    totals = {
        "valid": sum(
            category["valid"] for manual in manuals for category in manual["categories"]
        ),
        "to_review": sum(
            category["to_review"]
            for manual in manuals
            for category in manual["categories"]
        ),
        "missing": sum(
            category["missing"]
            for manual in manuals
...
async def search_private_library(
    ...
    if (
        source_filename
        and source_filename not in _lib_svc.available_reference_manuals()
    ):
        raise HTTPException(
            status_code=400, detail="Manuale non disponibile nella biblioteca privata"
        )
    records = await private_reference_records(user.user_id, db=db)
    if requested_types:
        records = [
    ...
    if review_only:
        records = [
            record for record in records if reference_review_state(record) == "review"
        ]
...
async def import_private_library(
    body: ReferenceImportInput,
    user: User = Depends(require_premium),
    db: SupabaseDatabase = Depends(get_db),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail="Importazione della biblioteca privata non riuscita"
        ) from exc


@router.get("/library/{reference_id}")
async def get_private_reference(
    reference_id: str,
    user: User = Depends(get_current_user),
    db: SupabaseDatabase = Depends(get_db),
):
    record = await db.private_reference_records.find_one(
...
async def apply_private_reference(
    reference_id: str,
    user: User = Depends(get_current_user),
    db: SupabaseDatabase = Depends(get_db),
):
...
async def retry_private_reference_translation_endpoint(
    ...
    return reference_summary(
        await retry_private_reference_translation(user.user_id, reference_id, db=db)
    )
...
async def review_private_reference(
    ...
    requested_description = (
        body.description if body.description is not None else body.corrected_description
    )
    requested_full_text = (
        body.full_text if body.full_text is not None else body.corrected_full_text
    )
    requested_attributes = (
        body.attributes if body.attributes is not None else body.corrected_attributes
    )
    if requested_name is not None:
        name = requested_name.strip()
        if not name:
            raise HTTPException(
                status_code=422, detail="Il nome del contenuto non può essere vuoto"
            )
        update_fields["name"] = name
        update_fields["normalized_name"] = normalize_reference_name(name)
        corrections["name"] = name
    if requested_description is not None:
        update_fields["description"] = compact_text(
            requested_description, maximum=12000
        )
    ...
    if (
        ...
        and (record.get("ai_review_corrections") or {}).get("canonical_invalidated")
        is True
    ):
    ...
    effective_full_text = update_fields.get(
        "full_text", record.get("full_text", "")
    ).strip()
    if body.review_status == "verified" and (
        not effective_name or not effective_full_text
    ):
        raise HTTPException(
            status_code=422,
    ...
    try:
        ...
        raise
    updated = await db.private_reference_records.find_one(
        {"id": reference_id, "user_id": user.user_id}
    )
