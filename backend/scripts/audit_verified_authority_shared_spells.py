...
def summarize_verified_authority_shared_spell_evidence(
    ...
    def classify_identity_provenance(identity: tuple[str, str]) -> str:
        ...
        for identity_record in identity_records:
            ...
            provenance_filenames.update(
                _ref_filenames(identity_record.get("source_refs"))
            )
        ...
        for filename in provenance_filenames:
            ...
            if _class_pack(filename) is not None and not exact_sources:
                continue
            alias = _normalized_filename_alias_key(filename)
            matched_sources = exact_sources or (
                sources_by_alias.get(alias, []) if alias else []
            )
    ...
    for pair in base["unidirectional_pairs"]:
        ...
        active_matches = sum(
            ...
            if all(_identity(record))
            and _identity(record) in active_authority_identities
        )
        verified_matches = sum(
            1
            for record in owner_records
            if all(_identity(record))
            and _identity(record) in verified_authority_identities
        )
        row = dict(pair)
        row["mixed_records_with_active_authority_same_identity"] = active_matches
        row["mixed_records_with_verified_active_authority_same_identity"] = (
            verified_matches
        )
        row["mixed_records_without_verified_active_authority_same_identity"] = (
            len(owner_records) - verified_matches
        )
        row[
            "verified_active_authority_same_identity_is_stronger_supporting_evidence"
        ] = verified_matches > 0
        ...
        row["classification_requires_review"] = (
            evidence_classification == AMBIGUOUS_REVIEW
        )
        ...
        if verified_matches > 0:
            ...
            row["residual_review_reason"] = (
                "active_authority_identity_present_but_not_verified"
            )
        else:
    ...
    residual_reason_counts = Counter(
        row["residual_review_reason"] for row in residual_shared_rows
    )
    ...
    return {
        ...
        "verified_active_authority_structured_identities": len(
            verified_authority_identities
        ),
        ...
        "shared_class_card_candidate_pairs_without_verified_active_authority_identity_evidence": len(
            residual_shared_rows
        ),
        "shared_class_card_candidate_pairs_requiring_classification_review": classification_review_rows,
        "residual_shared_class_card_candidate_pairs_by_reason": dict(
...
async def _run() -> int:
    ...
    print(
        json.dumps(
            summarize_verified_authority_shared_spell_evidence(jobs, sources, records),
            sort_keys=True,
        )
    )
    return 0
