...
def _is_additive_counter(key: str, value: object) -> bool:
    return isinstance(value, int) and (key in _ADDITIVE_KEYS or key.startswith("monster_"))
...
def combine_reports(reports: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        windows.append(
            {
                ...
                "quality_pages_evaluated": int(report.get("quality_pages_evaluated") or 0),
                "monster_candidates_primary": int(report.get("monster_candidates_primary") or 0),
                "monster_candidates_comparison": int(report.get("monster_candidates_comparison") or 0),
    ...
    result: dict[str, Any] = {
        ...
        "page_count_requested_total": sum(window["page_count_requested"] for window in windows),
        "window_count": len(windows),
        "windows": windows,
        "record_types": dict(sorted(record_types.items())),
        "page_quality": sorted(page_quality, key=lambda item: int(item.get("page") or 0)),
        "aggregation_note": (
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Combine privacy-safe bounded OCR pilot reports")
