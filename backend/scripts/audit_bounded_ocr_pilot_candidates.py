...
def summarize_bounded_ocr_pilot_candidates(
    ...
    for source in sources:
        ...
        sha_logical_ids = (
            {
                _norm(item.get("logical_source_id"))
                for item in sources_by_sha.get(sha, [])
        ...
        extraction_peers = sum(
            _norm(peer.get("source_role"))
            in {"extraction_aid", "visual_aid", "ingest_copy"}
    ...
    counts = {
        name: sum(row["classification"] == name for row in rows) for name in priority
    }
    ...
    return {
        ...
        "all_sources": rows,
        "recommended_pilot_source_id": recommended["source_id"]
        if recommended
        else None,
        "recommended_next_step": recommended["recommended_review_path"]
        if recommended
        else None,
...
async def _run() -> int:
    ...
    print(
        json.dumps(
            summarize_bounded_ocr_pilot_candidates(sources, jobs), sort_keys=True
        )
    )
    return 0
