...
def canonical_group_key(record: dict, ruleset: str = "2014") -> str:
    ...
    if record_ruleset(record) != ruleset:
        raise ValueError(
            f"La fonte appartiene al ruleset D&D 5e {record_ruleset(record)}"
        )
...
def record_ruleset(record: dict) -> str:
    ...
    for item in metadata:
        value = str(
            item.get("ruleset") or item.get("edition") or item.get("edition_year") or ""
        )
        if "2024" in value:
            return "2024"
        if "2014" in value:
...
def source_authority(record: dict) -> tuple[int, str, bool]:
    ...
    official = any(
        item.get("official") or item.get("official_source") for item in metadata
    )
    if revision:
        return 50, "official_revision", True
    known_official = any(
            ...
            entry.update(
                {
                    ...
                    "temporal_role": "later_revision"
                    if later
                    else ("historical" if later_exists else "contemporaneous"),
                }
            )
...
def _best(records: list[dict]) -> dict:
    return sorted(
        records, key=lambda row: (-source_authority(row)[0], str(row.get("id", "")))
    )[0]
...
def _record_has_blocking_uncertainty(record: dict) -> bool:
    ...
    if bool(flags & _VISUAL_REVIEW_FLAGS) or record.get("translation_status") in {
        "failed",
        "processing",
    }:
        return True
    ...
    supplied = [
        {
            ...
            "source_refs": provenance([r]),
            "authority": {
                "rank": source_authority(r)[0],
                "class": source_authority(r)[1],
            },
        }
        for r in candidates
    ]
...
async def canonicalize_group(
    records: list[dict], ruleset: str = "2014", comparator: Comparator | None = None
) -> dict:
    key = canonical_group_key(records[0], ruleset)
    selected = _best(records)
    state, confidence, notes, conflicts = (
        "verified",
        1.0,
        "Equivalent source records.",
        [],
    )
    requires_ai = not _equivalent(records) or any(
        _record_needs_ai_review(record) for record in records
    )
    if requires_ai:
        try:
            answer = await _compare(comparator or openai_comparator, records)
            selected_id = (
                answer.get("selected_source_record_id")
                if isinstance(answer, dict)
                else None
            )
            confidence = (
                float(answer.get("confidence", 0)) if isinstance(answer, dict) else 0
            )
            notes = str(answer.get("notes", "")) if isinstance(answer, dict) else ""
            conflicts = (
                answer.get("conflict_fields", []) if isinstance(answer, dict) else []
            )
            ...
            if source_authority(selected)[0] < highest_authority:
                raise ValueError(
                    "AI selected a source below the highest available authority"
                )
            ...
            state = (
                "verified"
                if requested == "verified" and confidence >= 0.8 and not conflicts
                else (
                    "conflict"
                    if requested == "conflict" or conflicts
                    else "low_confidence"
                )
            )
            if state == "verified" and _record_has_blocking_uncertainty(selected):
                ...
                notes = (
                    notes
                    + " Verifica visiva o seconda fonte necessaria prima dell'uso canonico."
                ).strip()
            ...
            state, confidence, notes, conflicts = (
                "conflict",
                0.0,
                "Comparator unavailable or invalid response.",
                [],
            )
    ...
    row = {
        ...
        "canonical_key": key,
        "reference_type": reference_effective_type(selected),
        "normalized_name": selected.get("normalized_name")
        or normalize_reference_name(selected.get("name", "")),
        "name": selected.get("name", ""),
        "description": selected.get("description", ""),
        "full_text": selected.get("full_text", ""),
        "attributes": selected.get("attributes", {}),
        "parent_class": selected.get("parent_class", ""),
        "parent_subclass": selected.get("parent_subclass", ""),
        "level": reference_effective_level(selected),
        "source_record_ids": [r["id"] for r in sorted(records, key=lambda r: r["id"])],
...
async def canonicalization_status(user_id: str, *, db) -> dict:
    ...
    for members in groups.values():
        ...
        state = next(
            (
                candidate
                for candidate in ("conflict", "low_confidence", "pending", "verified")
    ...
    return {
        ...
        "verified_groups": counts["verified"],
        "conflict_groups": counts["conflict"],
        "low_confidence_groups": counts["low_confidence"],
...
async def run_canonicalization(
    ...
    *,
    ...
    for record in records:
        ...
        if source_record_is_excluded(record):
            exclusions.append(
                (
                    record,
                    "Duplicate, superseded, misidentified or document source excluded.",
                )
            )
            continue
    ...
    processed = 0
    for record, reason in sorted(
        exclusions, key=lambda item: str(item[0].get("id", ""))
    ):
        if processed >= batch_size:
            break
        if (
            record.get("ai_review_status") == "excluded"
            and record.get("ai_review_notes") == reason
        ):
            continue
    ...
    for key in sorted(groups):
        ...
        existing = await db.private_reference_canonical.find_one(
        ...
        if existing:
            ...
            await db.private_reference_canonical.update_one(
                {"id": row["id"], "user_id": user_id}, {"$set": row}
            )
        else:
            await db.private_reference_canonical.insert_one(row)
        selected_id = next(
            ref["source_record_id"] for ref in row["source_refs"] if ref["selected"]
        )
        for member in members:
            ...
            await db.private_reference_records.update_one(
                {"id": member["id"], "user_id": user_id},
                {
                    "$set": {
                        "canonical_id": row["id"],
                        "ai_review_status": row["verification_status"],
                        "ai_confidence": row["confidence"],
                        "ai_review_model": row["verification_model"],
                        "ai_reviewed_at": row["updated_at"],
                        "ai_review_notes": row["verification_notes"],
                        "ai_review_corrections": {
        ...
        processed += 1
    return {
        **await canonicalization_status(user_id, db=db),
