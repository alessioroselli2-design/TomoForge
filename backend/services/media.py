...
async def cleanup_artwork(data: bytes, content_type: str) -> tuple[bytes, str]:
    ...
    image_value = (
        ...
        else getattr(result, "b64_json", None)
    )
    if image_value:
    ...
    image_url = (
        result.get("url") if isinstance(result, dict) else getattr(result, "url", None)
    )
    if image_url:
        image_response = await asyncio.to_thread(
            requests.get, image_url, timeout=(10, 120)
        )
    ...
    *,
    ...
    if cleanup:
        try:
            ...
            logger.warning(
                "Artwork cleanup skipped; preserving original artwork: %s", exc
            )
            ...
            if not data or not content_type.startswith("image/"):
                ...
                raise HTTPException(
                    status_code=502,
                    detail="La pulizia non ha restituito un'immagine valida",
                )
    ...
    return await save_file(
        path, data, content_type, user_id, cleaned_filename, db=db
    ), cleanup_notice
