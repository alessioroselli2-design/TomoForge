...
def manual_preload_summary(job: Optional[dict], page_count: Optional[int]) -> dict:
    ...
    return {
        ...
        "translation_processing_confirmed": bool(
            job.get("translation_processing_confirmed")
        ),
...
async def ensure_manual_preload_jobs(
    user_id: str, body: ManualPreloadInput, *, db=None
) -> list[dict]:
    ...
    if body.filename and body.filename not in manuals:
        raise HTTPException(
            status_code=400, detail="Il manuale richiesto non è disponibile localmente"
        )
    selected_manuals = (
        {body.filename: manuals[body.filename]} if body.filename else manuals
    )
    ...
    if collection is None:
        raise HTTPException(
            status_code=503,
            detail="Coda di indicizzazione non disponibile: applica prima la migrazione SQL",
        )
    ...
    for filename, path in selected_manuals.items():
        ...
        changed_source = bool(
            existing and existing.get("source_fingerprint") != fingerprint
        )
        status = existing.get("status") if existing else "queued"
        if duplicate_of:
            status = "failed"
        elif changed_source or (
            body.filename == filename
            and (body.enable_translation or body.enable_ocr or body.retry)
        ):
            status = "queued"
        ...
        if existing:
            if duplicate_of:
                ...
                payload.update(
                    {"lease_id": "", "lease_expires_at": 0, "last_error": ""}
                )
    ...
    for candidate in candidates:
        ...
        if claimed.matched_count:
            return await collection.find_one(
                {"id": candidate["id"], "user_id": user_id}
            )
    return None
        ...
        while True:
            ...
            try:
                renewed = await collection.update_one(
                    ...
                    {
                        "$set": {
                            "lease_expires_at": int(time.time())
                            + MANUAL_PRELOAD_LEASE_SECONDS,
                            "updated_at": utc_now(),
                        }
                    },
...
async def _drain_rate_limited_ids(records_collection: Any, query: dict) -> list[str]:
    ...
    while True:
        page = (
            await records_collection.find(query)
            .sort("id", 1)
            .to_list(_RATE_LIMIT_DRAIN_PAGE_SIZE, offset)
        )
...
async def _retry_rate_limited_translations(
    ...
    *,
    ...
    for attempt, delay in enumerate(delays):
        ...
        pending_ids = await _drain_rate_limited_ids(
            records_collection, rate_limit_query
        )
        if not pending_ids:
            return 0
        ...
        remaining_ids = await _drain_rate_limited_ids(
            records_collection, rate_limit_query
        )
        if not remaining_ids:
            return 0

    still_limited_ids = await _drain_rate_limited_ids(
        records_collection, rate_limit_query
    )
...
async def process_manual_preload_job(user_id: str, job: dict, *, db=None) -> None:
    ...
    if not page_count:
        await collection.update_one(
            owned_query,
            {
                "$set": {
                    "status": "failed",
                    "last_error": "manual_page_count_unavailable",
                    "lease_id": "",
                    "lease_expires_at": 0,
                    "updated_at": utc_now(),
                }
    ...
    try:
        ...
        if not checkpoint.matched_count:
            return False
        logger.warning(
            "Manual preload failed for %s page %s: %s", filename, current_page, exc
        )
        return
    if await _stop_renewal():
        return False
    ...
    source_report = next(
        (source for source in report.sources if source.get("filename") == filename), {}
    )
    ...
    if translation_rate_limited and records_collection is not None:
        ...
        retry_task = asyncio.create_task(
            _retry_rate_limited_translations(
                ...
                filename,
                records_collection,
                job_updater=_update_retry_state,
                db=_db,
            )
        )
    ...
    for job in jobs:
        ...
        if (
            ...
            if reset.matched_count:
                owners.add(job.get("user_id", ""))
            continue
        if job.get("status") == "processing" and int(
            job.get("lease_expires_at") or 0
        ) < int(time.time()):
            await collection.update_one(
                {
                    "id": job["id"],
